...
import { View, Text, StyleSheet, ScrollView, Dimensions, Pressable, Platform, ActivityIndicator, Modal } from 'react-native';
...
export default function GeneratedScheduleView() {
    ...
    for (let i = 0; i < materias.length; i++) {
      ...
      for (const horario of materia.horarios) {
        if (horario.dia === dia) {
          ...
          const horaFinInt = parseInt(horario.horaFin.split(':')[0]);
  ...
  }, [isResizing, isMobileScreen]);  return (
      ...
      <View style={styles.contentContainer}>
          ...
          <>
            <View style={[
              ...
              <GeneratedScheduleSidebar 
                ...
                onSelectHorario={setHorarioActual}
        ...
        {(!isMobileScreen || !activSidebar) && (
          <View style={[
            ...
            { width: (!isMobileScreen && activSidebar) ? `${100 - sidebarWidth}%` : '100%' }
          ]}>
            {loading ? (
                          ...
                          <View key={`${day}-${time}`} style={styles.cell}>
                            {celda ? (
                              <Pressable 
                                ...
                                onPress={() => {
                                  const materias = horarioActual?.materias || [];
                                  const materiaIndex = materias.findIndex(m => m.nrc === celda.materia.nrc);
                                  openMateriaDetails(celda.materia, materiaIndex);
                                }}
                              >
                                {celda.esPrimera && (
                                  <>
                                    ...
                                    <Text style={styles.cellNombre} numberOfLines={2}>
                                      {celda.materia.nombre}
                                    </Text>
                                    <Text style={styles.cellProfesor} numberOfLines={1}>
                ...
                <View style={styles.modalColumn}>
                  ...
                  <View style={styles.nrcContainer}>
                    ...
                    <Pressable
                      ...
                      onPress={() => copyToClipboard(selectedMateria.nrc, 'NRC')}
                    >
                      <Image 
